class SubcategoriesController < ApplicationController
  ...
  def show
    @stored_books = Book.all.joins("join subcategories
      on books.subcategory_id = subcategories.id").where("books.subcategory_id=?", @subcategory.id)
    @new_book = @subcategory.books.build
  ...
  def subcategory_params
    params.require(:subcategory).permit(:subcategory_topic, :subcategory_description, :category_id)
  end
end
